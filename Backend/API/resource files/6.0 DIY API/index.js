...
app.post('/jokes',(req,res)=>{
  ...
  res.json({message:"Your joke is sent succesfully",
            newJoke:jokes.slice(-1)
          });
});
//5. PUT a joke
app.put('/jokes/:id',(req,res)=>{
  const id=parseInt(req.params.id);
  //another method
  //const searchIndex=findIndex((joke)=>joke.id===id);
  jokes[id-1]={
    id:id,
...
//6. PATCH a joke(MUST CHECK SOLUTION)
...
app.delete('/jokes/:id',(req,res)=>{
  const id=parseInt(req.params.id);
  const searchIndex=jokes.findIndex((joke)=>joke.id===id);//returns -1 if no index found
  if(searchIndex>-1){
  jokes.splice(id-1,1);
  res.json({message:"joke succesfully deleted!"});
  }
  else{
    res.status(404)
       .json({error:`No joke found for ${id}`});
  }
})
//8. DELETE All jokes
app.delete('/all',(req,res)=>{
  ...
  if(key===masterKey){
    jokes=[];
    res.json({code:"Nuclear bomb detonated!"});
  }
  else{
    res.status(404)
       .json({error:"Inavlid Master Key!!!!!!!!"});
  }
})
app.listen(port, () => {
